...
EN_serverError_t updateAcount(void)
{
     ...
     {
          for(u8 i=0;i<record;i++) fprintf(file,"%d,%s,%s,%f\n",accountsDB[i].flag,accountsDB[i].pan,accountsDB[i].date,accountsDB[i].amount);
          flag=OK_S;
     }
     fclose(file);
     return flag;
...
logic loadAccountData(void)
{
     ...
     do
     {
          read=fscanf(file,"%d,%16[^,],%5[^,],%f,\n",&accountsDB[record].flag,accountsDB[record].pan,accountsDB[record].date,&accountsDB[record].amount);
          if (read==4)record++;
          else if (read!=5 && !feof(file)) printf("File format incorrect.\n");
